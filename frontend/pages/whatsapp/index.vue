        ...
        <div class="row g-3">
          ...
          <div class="col-md-6">
            ...
            <select v-model="sendForm.internship_group_id" class="form-select" required>
              ...
              <option v-for="g in groups" :key="g.id as number" :value="g.id">{{ g.name }}</option>
            </select>
          </div>
          <div class="col-12">
          ...
          <template v-if="sendForm.send_type === 'manual'">
            <div class="col-12">
              ...
              <select v-model="sendForm.student_ids" class="form-select" multiple size="8">
                <option v-for="s in collegeStudents" :key="s.id as number" :value="s.id">
        ...
        <div v-if="previewStudents.length" class="mt-4">
          ...
          <div class="table-responsive border rounded">
            <table class="table table-sm table-striped mb-0">
              ...
              <tbody>
                <tr v-for="s in previewStudents" :key="s.id as number">
                  <td>{{ s.id }}</td>
      ...
      <div class="card table-card">
        <div class="table-responsive">
          <table class="table table-hover mb-0">
            ...
            <tbody>
              <tr v-for="m in messages" :key="m.id as number">
                ...
                <td>
                  <button v-if="m.status === 'failed'" class="btn btn-sm btn-link" @click="resend(m.id as number)">Resend</button>
                </td>
              </tr>
...
const tab = ref<'send' | 'logs'>('send')
const colleges = ref<Record<string, unknown>[]>([])
const groups = ref<Record<string, unknown>[]>([])
const collegeStudents = ref<Record<string, unknown>[]>([])
const previewStudents = ref<Record<string, unknown>[]>([])
...
const sendForm = reactive({
  college_id: '',
  internship_group_id: '',
  send_type: 'manual' as 'manual' | 'range',
  student_ids: [] as number[],
  start_student_id: null as number | null,
  end_student_id: null as number | null,
})

const filters = reactive({ status: '', internship_group_id: '' })

const statusBadge = (s: unknown) => {
  const m: Record<string, string> = { sent: 'text-bg-success', failed: 'text-bg-danger', pending: 'text-bg-warning' }
  return m[String(s)] || 'text-bg-secondary'
}

onMounted(async () => {
  const [c, g] = await Promise.all([
    apiFetch<{ success: boolean; data: Record<string, unknown>[] }>('/admin/colleges/dropdown'),
    apiFetch<{ success: boolean; data: Record<string, unknown>[] }>('/admin/groups?per_page=100'),
  ])
...
const loadPreview = async () => {
  ...
  try {
    ...
    toast.show(`${res.data.count} student(s) ready to send.`, 'info')
  } catch (e) {
...
const send = async () => {
  if (!sendForm.internship_group_id) {
    toast.show('Select a WhatsApp group.', 'danger')
    return
  }
  sending.value = true
  try {
    ...
    toast.show(res.message || 'Messages queued.', 'success')
    tab.value = 'logs'
    await load()
  } catch (e) {
    toast.show(parseApiError(e), 'danger')
...
const retryAll = async () => {
  await retryFailed()
  toast.show('Failed messages re-queued.', 'info')
  await load()
}

const resend = async (id: number) => {
  await resendMessage(id)
  toast.show('Message re-queued.', 'success')
  await load()
}
