  ...
  <div>
    <div class="d-flex flex-wrap justify-content-between align-items-center gap-2 mb-3">
      ...
      <div class="d-flex gap-2">
        ...
        <NuxtLink v-if="canCreate" to="/students/create" class="btn btn-primary btn-sm">
    ...
    <DataTable :columns="columns" :rows="rows">
      <template #actions="{ row }">
        <NuxtLink v-if="canEdit" :to="`/students/${row.id}`" class="btn btn-sm btn-outline-primary me-1">Edit</NuxtLink>
        <button
...
const canCreate = computed(() => can('student_create'))
const canEdit = computed(() => can('student_edit'))
const canApprove = computed(() => can('student_approve'))
const canExport = computed(() => can('student_view'))
...
const { data, pending, error, refresh } = await useAsyncData('internship-students-list', fetchStudents, { server: false })
...
const rows = computed(() =>
  (data.value?.items ?? []).map((s) => ({
    ...
    college_name: s.college_name || (s.college as Record<string, string>)?.college_name || '—',
...
const approve = async (id: unknown) => {
  try {
