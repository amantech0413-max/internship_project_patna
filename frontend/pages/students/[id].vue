<template>
  <div class="max-w-3xl mx-auto w-full">
    <div v-if="loadError" class="bg-red-50 border border-red-200 rounded-xl p-6 text-center">
      ...
      <NuxtLink to="/students" class="inline-block mt-4 text-blue-700 font-medium">← Back to Students</NuxtLink>
    </div>
    ...
    <template v-else-if="student">
      ...
      <div class="bg-white rounded-xl border shadow-sm p-4 sm:p-6">
        ...
        <StudentForm
          ...
          show-status
          show-rejection
          :photo-preview-url="(student.photo_url as string) || null"
        ...
        >
          <template #actions>
            ...
            <div class="flex flex-col sm:flex-row gap-3 pt-4">
              ...
              <NuxtLink to="/students" class="w-full sm:w-auto px-6 py-3 border rounded-lg text-center">Back</NuxtLink>
            </div>
          </template>
        </StudentForm>
      </div>
...
const { data: student, pending, refresh, error: fetchErr } = await useAsyncData(
  `student-edit-${route.params.id}`,
  async () => {
...
const statusClass = computed(() => {
  const s = student.value?.status as string
