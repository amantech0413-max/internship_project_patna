  ...
  <div>
    ...
    <div class="card table-card mb-3">
      <div class="card-body row g-2 align-items-end">
        ...
        <div class="col-md-2">
          <NuxtLink to="/entry" class="btn btn-primary w-100">+ Staff Entry</NuxtLink>
        </div>
      </div>
    </div>
    ...
    <div class="card table-card">
      <div class="table-responsive">
        <table class="table table-hover mb-0">
          ...
          <tbody>
            <tr v-for="(row, idx) in rows" :key="row.id as number">
              <td>{{ rowIndex(idx) }}</td>
              <td class="fw-medium">{{ row.student_name }}</td>
              <td>{{ row.mobile_number }}</td>
              <td>{{ (row.college as Record<string, string>)?.college_name || '—' }}</td>
              <td>{{ (row.added_by_user as Record<string, string>)?.name || (row.creator as Record<string, string>)?.name || '—' }}</td>
...
const { data, pending, error, refresh } = await useAsyncData('staff-students-list', fetchList, { server: false })
...
const rowIndex = (idx: number) => ((meta.value?.current_page || 1) - 1) * (meta.value?.per_page || 15) + idx + 1
